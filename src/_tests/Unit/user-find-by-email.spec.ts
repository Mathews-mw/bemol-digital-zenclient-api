...
describe('Find user by email Use Case', () => {
	beforeEach(() => {
		...
		findUserByemailUseCase = new FindUserByEmailUseCase(userRepository, addressRepository, userAddressRepository);
	});

	it('Should be able to find an user by e-mail', async () => {
		...
		const user = await findUserByemailUseCase.execute({ email: 'john.doe@example.com' });
	...
	it('Should not be able to find a non-existent user e-mail', async () => {
		await expect(() => findUserByemailUseCase.execute({ email: 'john.doe@example.com' })).rejects.toBeInstanceOf(ResourceNotFoundError);
	});
});
